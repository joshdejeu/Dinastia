        ...
        simulation = d3
            ...
            .force(
                ...
                d3
                    .forceLink(links)
                    .id((d) => d.id)
                    .distance(500) // ← How long each link wants to be
                    .strength(0.2), // ← How *strongly* the link enforces that length
            )
            .force(
                "collision",
                d3
                    ...
                    .radius(
                        () =>
                            // Increase collision radius based on both width and height
                            Math.sqrt(nodeWidth ** 2 + nodeHeight ** 2) / 2 +
                            10,
                    )

                    .strength(1),
            )
            .force("y", d3.forceY(height / 2).strength(0.15)) // stronger vertical pull
            .force("x", d3.forceX().strength(0.01)) // very light horizontal pull
            // .force("charge", d3.forceManyBody().strength(-800))
            .alpha(0.03) // Start with less energy (instead of default 1)
            .alphaDecay(0.02) // Slow down cooling (default ~0.0228)
            .alphaMin(0.001) // When to stop simulation (default)
            .on("tick", ticked);
    }
